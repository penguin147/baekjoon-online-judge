...
string[] input_station, input_way;
int N, cnt = 0;
int[] station;
long[] way;
long result = 0;
...
input_station = new string[N];
station = new int[N];
input_way = new string[N - 1];
way = new long[N - 1];

input_way = Console.ReadLine().Split(' ');
input_station = Console.ReadLine().Split(' ');

for (int i = 0; i < N - 1; i++)
{
    way[i] = Convert.ToInt64(input_way[i]);
}

for (int i = 0; i < N; i++)
{
    station[i] = Convert.ToInt32(input_station[i]);
}
